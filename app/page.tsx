...
type SanMarPreviewItem = { styleNumber: string; productName: string; brand: string; category?: string; color: string; firstImageUrl?: string };
...
const getImageCandidates = (baseUrl: string | undefined, type: ImageType, view: ShirtView) => {
  if (!baseUrl) return [];
  const primary = swapImageToken(baseUrl, type, view);
  const fallbackType: ImageType = type === 'flat' ? 'model' : 'flat';
  const fallbackView: ShirtView = view === 'front' ? 'back' : 'front';
  return Array.from(new Set([
    primary,
    swapImageToken(baseUrl, fallbackType, view),
    swapImageToken(baseUrl, type, fallbackView),
    swapImageToken(baseUrl, fallbackType, fallbackView),
    baseUrl
  ]));
};
...
export default function Home() {
  ...
  useEffect(() => {
    const candidates = getImageCandidates(selectedPreview?.firstImageUrl, imageType, shirtView);
