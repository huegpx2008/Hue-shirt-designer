...
type SanMarPreviewItem = { styleNumber: string; productName: string; brand: string; category?: string; color: string; firstImageUrl?: string; frontModelImageUrl?: string; backModelImageUrl?: string; frontFlatImageUrl?: string; backFlatImageUrl?: string; productImage?: string };
type SanMarPreviewPayload = { sourceColumns: string[]; fieldMapping: Record<string, string>; rows: SanMarPreviewItem[] };
...
const getImageCandidates = (item: SanMarPreviewItem | undefined, type: ImageType, view: ShirtView) => {
  ...
  const fallbackTypePrimary = type === 'flat'
    ? (view === 'front' ? item.frontModelImageUrl : item.backModelImageUrl)
    : (view === 'front' ? item.frontFlatImageUrl : item.backFlatImageUrl);
  const tokenSource = direct || fallbackTypePrimary || item.firstImageUrl || item.productImage;
  ...
  return Array.from(new Set([direct, oppositeView, fallbackTypePrimary, item.firstImageUrl, item.productImage, ...tokenCandidates].filter(Boolean) as string[]));
};
...
export default function Home() {
  ...
  const previewPayload = useMemo<SanMarPreviewPayload>(() => sanMarPreview as SanMarPreviewPayload, []);
  const previewCatalog = useMemo<SanMarPreviewItem[]>(() => previewPayload.rows.slice(0, 25), [previewPayload]);
  ...
  const filteredPreviewCatalog = useMemo(() => {
    ...
    return groupedStyles.filter((group) => {
      ...
      return group.items.some((item) => [item.styleNumber, item.productName, item.brand, item.category || '', item.color]
        .some((value) => value.toLowerCase().includes(query)));
    });
  ...
  const selectedColorName = selectedPreview?.color || selectedProduct.availableColors.find((color) => color.value === shirtColor)?.name || shirtColor;
  ...
  const availableStyleColors = useMemo(() => selectedStyleItems.map((item) => ({ name: item.color, value: COLOR_MAP[item.color.toLowerCase()] || '#94a3b8' })), [selectedStyleItems]);

  useEffect(() => {
    if (!selectedPreview) return;
    const next = COLOR_MAP[selectedPreview.color.toLowerCase()];
