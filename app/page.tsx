...
export default function Home() {
  ...
  return <main className="min-h-screen w-full bg-slate-100 px-3 py-4 md:px-6 pb-28"><header className="mb-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 md:p-5"><h1 className="text-xl font-semibold tracking-tight md:text-2xl">Hue Shirt Design Studio</h1></header><nav className="sticky top-2 z-20 mb-3 flex gap-2 overflow-x-auto rounded-xl bg-white/95 p-2 shadow ring-1 ring-slate-200 md:hidden"><a href="#product" className="whitespace-nowrap rounded-lg bg-slate-900 px-3 py-2 text-sm font-medium text-white">Product</a><a href="#design" className="whitespace-nowrap rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium">Design</a><a href="#qty" className="whitespace-nowrap rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium">Quantity</a><a href="#estimate" className="whitespace-nowrap rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium">Estimate</a><a href="#quote" className="whitespace-nowrap rounded-lg bg-slate-100 px-3 py-2 text-sm font-medium">Quote</a></nav><div className="grid gap-4 lg:grid-cols-[88px_340px_360px_minmax(0,1fr)] lg:items-start"><aside className="hidden lg:flex lg:sticky lg:top-4 h-[calc(100vh-7.5rem)] flex-col gap-2 rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200"><a href="#product" className="flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><span className="text-base">1</span><span className="whitespace-nowrap">Product</span></a><a href="#design" className="flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><span className="text-base">2</span><span className="whitespace-nowrap">Design</span></a><a href="#qty" className="flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><span className="text-base">3</span><span className="whitespace-nowrap">Qty</span></a><a href="#estimate" className="flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><span className="text-base">4</span><span className="whitespace-nowrap">Estimate</span></a><a href="#quote" className="flex flex-col items-center rounded-lg px-2 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><span className="text-base">5</span><span className="whitespace-nowrap">Quote</span></a></aside><aside id="product" className="space-y-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 md:p-5 lg:sticky lg:top-4 lg:max-h-[calc(100vh-7.5rem)] lg:overflow-y-auto"><h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">1) Choose Product</h2><input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search style, product, brand, category, color, or image URL" className="w-full rounded-lg border px-3 py-2 text-sm" /><div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><select value={brandFilter} onChange={(event) => setBrandFilter(event.target.value)} className="rounded-lg border px-2 py-1.5 text-xs"><option value="all">All Brands</option>{brandOptions.map((brand) => <option key={brand} value={brand}>{brand}</option>)}</select><select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="rounded-lg border px-2 py-1.5 text-xs"><option value="all">All Categories</option>{categoryOptions.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select><select value={sortOption} onChange={(event) => setSortOption(event.target.value as 'style' | 'name' | 'brand')} className="rounded-lg border px-2 py-1.5 text-xs"><option value="style">Style number A-Z</option><option value="name">Product name A-Z</option><option value="brand">Brand A-Z</option></select></div>{hasActiveCatalogFilters ? <button onClick={clearCatalogFilters} className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs">Clear Filters</button> : null}<div className="space-y-0.5 text-xs text-slate-500"><p>Total generated rows loaded: {totalSampleRowsLoaded}</p><p>Total unique styles: {totalUniqueStyles}</p><p>Products shown: {pagedPreviewCatalog.length}</p><p>Total results: {sortedPreviewCatalog.length}</p><p>Active category: {categoryFilter === 'all' ? 'All Categories' : (CHUNKED_CATEGORY_LABELS[categoryFilter as CategoryChunkSlug] || categoryFilter)}</p><p>Catalog mode: generated category chunk</p><p>{categoryCatalogStatus}</p>{isCategoryCatalogLoading ? <p>{CHUNKED_CATEGORY_LOAD_MESSAGES[categoryFilter as CategoryChunkSlug] || 'Loading Catalog…'}</p> : null}</div>{sortedPreviewCatalog.length === 0 ? <div className="rounded-lg border border-amber-300 bg-amber-50 p-2 text-xs text-amber-900"><p className="font-semibold">No products match your current search and filters.</p><p>Search: {searchQuery || '(empty)'}</p><p>Brand: {brandFilter === 'all' ? 'All Brands' : brandFilter}</p><p>Category: {categoryFilter === 'all' ? 'All Categories' : categoryFilter}</p><button onClick={clearCatalogFilters} className="mt-2 rounded border border-amber-400 bg-white px-2 py-1">Clear Filters</button></div> : null}<div className="max-h-[560px] space-y-2 overflow-y-auto pr-1">{pagedPreviewCatalog.map((group, index) => { const catalogIndex = previewCatalog.findIndex((product) => product.styleNumber === group.styleNumber); return <button key={`${group.styleNumber}-${index}`} onClick={() => setSelectedPreviewId(catalogIndex)} className={`w-full rounded-lg border px-3 py-3 text-left text-sm ${selectedPreviewId === catalogIndex ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white'}`}><div className="flex gap-2"><img src={getProductCardImage(group.items[0])} alt={group.name} onError={(event) => { event.currentTarget.onerror = null; event.currentTarget.src = PRODUCT_IMAGE_PLACEHOLDER; }} className="h-16 w-16 shrink-0 rounded border object-cover bg-slate-100 p-1" /><div className="min-w-0"><p className="font-semibold">{group.styleNumber}</p><p className="truncate text-slate-700">{group.name}</p><p className="truncate text-slate-500">{group.brand}</p><p className="text-slate-500">{group.category || 'Uncategorized'} • {group.items.length} colors</p></div></div></button>; })}</div>{hasMoreProducts ? <button onClick={() => setVisibleProductCount((prev) => prev + PRODUCTS_PAGE_SIZE)} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs">Load More Products</button> : null}</aside><aside id="design" className="space-y-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 md:p-5 lg:sticky lg:top-4 lg:max-h-[calc(100vh-7.5rem)] lg:overflow-y-auto"><h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">2. Design Shirt</h2><section><p className="mb-2 text-sm font-medium">Mockup Type Override</p><select value={selectedProductId} onChange={(e) => setSelectedProductId(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm">{SAMPLE_PRODUCT_CATALOG.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></section><section><p className="mb-2 text-sm font-medium">View</p><div className="grid grid-cols-2 gap-2">{(['front', 'back'] as ShirtView[]).map((view) => <button key={view} onClick={() => setShirtView(view)} className={`rounded-lg border px-3 py-2 text-sm capitalize ${shirtView === view ? 'bg-slate-900 text-white' : ''}`}>{view}</button>)}</div></section><section><p className="mb-1 text-sm font-medium">Print Locations</p><div className="grid grid-cols-2 gap-1.5">{selectedProduct.defaultPrintLocations.map((location) => <label key={location} className="flex items-center gap-1.5 rounded border px-2 py-1 text-[11px]"><input type="checkbox" checked={selectedPrintLocations.includes(location)} onChange={(e) => { const checked = e.target.checked; setSelectedPrintLocations((prev) => { const next = checked ? Array.from(new Set([...prev, location])) : prev.filter((p) => p !== location); return next.length ? next : [location]; }); if (!selectedPrintLocations.includes(location)) setPrintLocation(location); }} /><span>{PRINT_AREA_CONFIG[location].label}</span></label>)}</div><select value={printLocation} onChange={(e) => setPrintLocation(e.target.value as PrintLocation)} className="mt-2 w-full rounded-lg border px-3 py-2 text-sm">{selectedPrintLocations.map((location) => <option key={location} value={location}>{PRINT_AREA_CONFIG[location].label}</option>)}</select><textarea value={activeLocationSettings.notes} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], notes: e.target.value } }))} placeholder="Notes for this location" rows={2} className="mt-2 w-full rounded-lg border px-3 py-2 text-sm" /></section><section><p className="mb-2 text-sm font-medium">Print Artboard</p><p className="mb-2 text-xs text-slate-500">Adjust the print artboard to line up with the shirt mockup. Final print placement may be adjusted during production.</p><div className="mb-2 flex gap-2"><button onClick={centerArtboardOnShirt} className="rounded-lg border px-2 py-1 text-xs">Center on Shirt</button><button onClick={() => setShowPrintArtboard((prev) => !prev)} className="rounded-lg border px-2 py-1 text-xs">{showPrintArtboard ? 'Hide Print Artboard' : 'Show Print Artboard'}</button></div><select value={printSizePreset} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], printSizePreset: e.target.value as PrintSizePreset } }))} className="w-full rounded-lg border px-3 py-2 text-sm"><option value="left-chest-3_5">Left Chest: 3.5" wide</option><option value="standard-front-10">Standard Front: 10" wide</option><option value="large-front-12">Large Front: 12" wide</option><option value="full-back-12">Full Back: 12" wide</option><option value="custom">Custom Size</option></select>{printSizePreset === 'custom' ? <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2"><input type="number" min="0" step="0.1" value={customPrintWidthInches} onChange={(event) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], customPrintWidthInches: event.target.value } }))} placeholder="Width (inches)" className="w-full rounded-lg border px-3 py-2 text-sm" /><input type="number" min="0" step="0.1" value={customPrintHeightInches} onChange={(event) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], customPrintHeightInches: event.target.value } }))} placeholder="Height (optional inches)" className="w-full rounded-lg border px-3 py-2 text-sm" /></div> : null}<p className="mt-2 text-xs text-slate-500">Print size is approximate and may be adjusted during production.</p><button onClick={() => setShowAdvancedArtboard((prev) => !prev)} className="mt-2 w-full rounded-lg border border-slate-300 bg-slate-50 px-2 py-1.5 text-xs text-left">{showAdvancedArtboard ? 'Hide Advanced Artboard Controls' : 'Advanced Artboard Controls'}</button>{showAdvancedArtboard ? <><div className="mt-2 grid grid-cols-2 gap-1.5"><button onClick={() => applyArtboardPreset('standard-full-front')} className="rounded-lg border px-2 py-1 text-xs">Standard Full Front</button><button onClick={() => applyArtboardPreset('large-full-front')} className="rounded-lg border px-2 py-1 text-xs">Large Full Front</button><button onClick={() => applyArtboardPreset('left-chest')} className="rounded-lg border px-2 py-1 text-xs">Left Chest</button><button onClick={() => applyArtboardPreset('full-back')} className="rounded-lg border px-2 py-1 text-xs">Full Back</button><button onClick={() => applyArtboardPreset('sleeve')} className="rounded-lg border px-2 py-1 text-xs">Sleeve</button><button onClick={() => applyArtboardPreset('reset-default')} className="rounded-lg border px-2 py-1 text-xs">Reset Default</button></div><div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2"><label className="text-xs">X Position<input type="number" value={Math.round(designArea.left)} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], artboard: { ...prev[printLocation].artboard, left: Number(e.target.value) || 0 } } }))} className="mt-1 w-full rounded-lg border px-2 py-1.5 text-sm" /></label><label className="text-xs">Y Position<input type="number" value={Math.round(designArea.top)} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], artboard: { ...prev[printLocation].artboard, top: Number(e.target.value) || 0 } } }))} className="mt-1 w-full rounded-lg border px-2 py-1.5 text-sm" /></label><label className="text-xs">Width<input type="number" min="20" value={Math.round(designArea.width)} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], artboard: { ...prev[printLocation].artboard, width: Math.max(20, Number(e.target.value) || 20) } } }))} className="mt-1 w-full rounded-lg border px-2 py-1.5 text-sm" /></label><label className="text-xs">Height<input type="number" min="20" value={Math.round(designArea.height)} onChange={(e) => setLocationSettings((prev) => ({ ...prev, [printLocation]: { ...prev[printLocation], artboard: { ...prev[printLocation].artboard, height: Math.max(20, Number(e.target.value) || 20) } } }))} className="mt-1 w-full rounded-lg border px-2 py-1.5 text-sm" /></label></div></> : null}</section><section className="space-y-2"><input value={textValue} onChange={(event) => setTextValue(event.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm" placeholder="Type your text" /><button onClick={addText} className="w-full rounded-lg bg-slate-900 px-3 py-2 text-sm text-white">Add Text</button><label className="block cursor-pointer rounded-lg border border-dashed p-3 text-center text-sm">Choose Image<input onChange={onUploadImage} className="hidden" type="file" accept="image/*" /></label></section><section><p className="mb-2 text-sm font-medium">Color</p><div className="grid grid-cols-2 gap-2">{availableStyleColors.map((color) => <button key={color.name} type="button" onClick={() => { const match = selectedStyleItems.find((item) => item.colorName === color.name); if (match) { const idx = previewCatalog.findIndex((entry) => entry.styleNumber === match.styleNumber && entry.colorName === match.colorName); if (idx >= 0) setSelectedPreviewId(idx); } setShirtColor(color.value); }} className={`flex items-center gap-2 rounded-lg border px-2 py-1.5 text-left text-xs ${shirtColor === color.value ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-300' : 'border-slate-300 bg-white'}`} title={color.name}>{color.swatchUrl ? <img src={color.swatchUrl} alt={color.name} className="h-6 w-6 rounded-full border object-cover" /> : <span className="h-6 w-6 rounded-full border" style={{ background: color.value }} />}<span className="truncate">{color.name}</span></button>)}</div></section><section><p className="mb-2 text-sm font-medium">Image Type</p><div className="grid grid-cols-2 gap-2">{(["flat", "model"] as ImageType[]).map((type) => <button key={type} onClick={() => setImageType(type)} className={`rounded-lg border px-3 py-2 text-sm capitalize ${imageType === type ? "bg-slate-900 text-white" : ""}`}>{type === "flat" ? "Flat/Product" : "Model/Worn"}</button>)}</div></section><section className="rounded-lg border bg-slate-50 p-3 text-xs"><p className="mb-2 font-semibold">Selected Catalog Product</p>{(resolvedImageUrl || selectedPreview?.productImageUrl) ? <img src={resolvedImageUrl || selectedPreview?.productImageUrl} alt={selectedPreview?.productName || 'Selected product'} className="mb-2 h-32 w-full rounded border object-contain bg-white" /> : null}<p><span className="font-medium">Style:</span> {selectedPreview?.styleNumber || 'N/A'}</p><p><span className="font-medium">Name:</span> {selectedPreview?.productName || 'N/A'}</p><p><span className="font-medium">Brand:</span> {selectedPreview?.brand || 'N/A'}</p><p><span className="font-medium">Category:</span> {selectedPreview?.category || 'N/A'}</p><p><span className="font-medium">Selected Color:</span> {selectedPreview?.colorName || 'N/A'}</p><p><span className="font-medium">Available Sizes:</span> {selectedPreview?.availableSizes?.length ? selectedPreview.availableSizes.join(', ') : 'N/A'}</p>{imageFallbackUsed ? <p className="mt-1 text-amber-700">Image fallback used</p> : null}</section><section className="rounded-lg border bg-slate-50 p-3"><p className="mb-2 text-sm font-medium">3. Enter Quantities</p><div id="qty" className="grid grid-cols-2 gap-2 sm:grid-cols-4">{SIZE_FIELDS.map((size) => <label key={size} className="text-xs"><span className="mb-1 block font-medium">{size}</span><input type="number" min={0} value={sizeQuantities[size]} onChange={(event) => setSizeQuantities((prev) => ({ ...prev, [size]: Math.max(0, Number(event.target.value) || 0) }))} className="w-full rounded border px-2 py-2 text-sm" /></label>)}</div><p className="mt-2 text-xs font-semibold">Total Quantity: {totalQuantity}</p><label className="mt-3 block text-xs font-medium">Print Method<select value={printMethod} onChange={(event) => setPrintMethod(event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5 text-sm"><option>Not sure / Recommend for me</option><option>DTF</option><option>Screen Print</option></select></label><label className="mt-2 block text-xs font-medium">Image Complexity<select value={imageComplexity} onChange={(event) => setImageComplexity(event.target.value)} className="mt-1 w-full rounded border px-2 py-1.5 text-sm"><option>Simple 1 color</option><option>2-3 colors</option><option>Full color / photo</option></select></label><div className="mt-3 rounded-md bg-white p-2 text-xs text-slate-700"><div className="mb-2 flex items-center justify-between gap-2"><p className="font-semibold">{printMethod === 'Not sure / Recommend for me' ? 'Recommended method' : 'Selected method'}</p><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${recommendationBadgeClass}`}>{printRecommendation.badge}</span></div><p><span className="font-medium">{printMethod === 'Not sure / Recommend for me' ? printRecommendation.method : printMethod}</span></p><p className="mt-1">{printMethod === 'Not sure / Recommend for me' ? printRecommendation.reason : `You selected ${printMethod}.`}</p>{printMethod !== 'Not sure / Recommend for me' && manualMethodWarning ? <p className="mt-1 text-amber-700">{manualMethodWarning}</p> : null}<p className="mt-1 text-slate-500">Final method may change after artwork review.</p></div></section><section id="quote" className="rounded-lg border bg-slate-50 p-4"><p className="mb-2 text-sm font-medium">5. Quote Request Info</p><div className="space-y-2 text-xs"><input value={customerInfo.name} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, name: event.target.value }))} placeholder="Name" className="w-full rounded border px-2 py-1.5" /><input value={customerInfo.organization} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, organization: event.target.value }))} placeholder="Business / Organization" className="w-full rounded border px-2 py-1.5" /><input value={customerInfo.email} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, email: event.target.value }))} placeholder="Email" className="w-full rounded border px-2 py-1.5" /><input value={customerInfo.phone} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, phone: event.target.value }))} placeholder="Phone" className="w-full rounded border px-2 py-1.5" /><label className="block font-medium">Needed-by Date<input type="date" value={customerInfo.neededByDate} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, neededByDate: event.target.value }))} className="mt-1 w-full rounded border px-2 py-1.5" /></label><textarea value={customerInfo.notes} onChange={(event) => setCustomerInfo((prev) => ({ ...prev, notes: event.target.value }))} placeholder="Notes" rows={3} className="w-full rounded border px-2 py-1.5" /></div></section><section id="estimate" className="rounded-lg border border-indigo-200 bg-indigo-50 p-4 text-sm"><p className="font-semibold text-indigo-900">4. Review Estimate</p><p className="mt-1">Ready to Request Quote: <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${quoteReadinessClass}`}>{quoteReadiness}</span></p><p>Required fields: Name, Email or Phone, Product, Quantity</p>{missingQuoteRequirements.length > 0 ? <p className="mt-1 text-amber-700">Missing: {missingQuoteRequirements.join(', ')}</p> : <p className="mt-1 text-emerald-700">All quote request requirements are complete.</p>}<p className="mt-2 font-semibold text-indigo-900">Customer Info</p><p>Name: {customerInfo.name || 'N/A'}</p><p>Business / Organization: {customerInfo.organization || 'N/A'}</p><p>Email: {customerInfo.email || 'N/A'}</p><p>Phone: {customerInfo.phone || 'N/A'}</p><p>Needed-by Date: {customerInfo.neededByDate || 'N/A'}</p><p className="mt-2 font-semibold text-indigo-900">Product Info</p><p>Selected Product: {selectedProductName}</p><p>Selected Color: {selectedColorName}</p><p className="mt-2 font-semibold text-indigo-900">Design Info</p><p>Print Location: {selectedPrintLocationLabel}</p><p>Print Size: {selectedPrintSizeLabel}</p><p>Shirt View: {shirtView}</p><p>Image Complexity: {imageComplexity}</p><p>Design Preview Status: {designPreviewStatus}</p><p>Design Preview Captured: {capturedDesignPreview ? 'Yes' : 'No'}</p>{capturedDesignPreview ? <img src={capturedDesignPreview} alt="Captured design preview" className="mt-2 h-20 w-20 rounded border object-contain bg-white" /> : null}<p>Design Export Status: Ready ({shirtView} / {printLocation})</p><p className="mt-2 font-semibold text-indigo-900">Quantity Breakdown</p><p>Total Quantity: {totalQuantity}</p><p>Size Breakdown: {sizeBreakdown}</p><p className="mt-2 font-semibold text-indigo-900">Print Method Recommendation</p><p>{recommendationByCost.recommendedMethod === 'dtf' ? 'DTF' : 'Screen Print'}</p><p className="mt-2 font-semibold text-indigo-900">Estimate Only Pricing</p><p>Setup ${estimatedSetupFee.toFixed(2)} • Decoration ${estimatedDecorationCost.toFixed(2)} • Per Shirt ${estimatedPerShirt.toFixed(2)}</p><p className="mt-2 font-semibold text-indigo-900">Notes</p><p>{customerInfo.notes || 'N/A'}</p><p className="mt-2 text-indigo-800">This is an estimate only. Final pricing may change after artwork review, garment availability, and exact production requirements.</p><div className="mt-3 rounded-md border border-slate-200 bg-white p-2"><p className="mb-2 text-xs font-semibold text-slate-700">Draft</p><div className="grid grid-cols-1 gap-2 sm:grid-cols-3"><button onClick={saveDraftToLocal} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">Save Draft</button><button onClick={loadDraftFromLocal} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">Load Draft</button><button onClick={clearDraftFromLocal} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm">Clear Draft</button></div>{draftStatus ? <p className="mt-2 text-xs text-slate-600">{draftStatus}</p> : null}</div><div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2"><button onClick={captureDesignPreview} className="rounded-lg border border-indigo-400 bg-white px-3 py-2 text-sm sm:col-span-2">Capture Design Preview</button><button onClick={copyQuoteSummary} className="rounded-lg bg-slate-900 px-3 py-2 text-sm text-white">Copy Quote Summary</button><button onClick={generateEmailSummary} className="rounded-lg border border-indigo-500 bg-white px-3 py-2 text-sm">Generate Email Summary</button><button onClick={copyEmailSummary} className="rounded-lg border border-indigo-500 bg-white px-3 py-2 text-sm">Copy Email Summary</button><button onClick={openEmailRequest} className="rounded-lg border border-indigo-500 bg-white px-3 py-2 text-sm">Open Email Request</button><button onClick={() => exportQuoteSummary('txt')} className="rounded-lg border border-slate-400 bg-white px-3 py-2 text-sm">Export Quote Summary (.txt)</button><button onClick={() => exportQuoteSummary('json')} className="rounded-lg border border-slate-400 bg-white px-3 py-2 text-sm sm:col-span-2">Export Quote Summary (.json)</button><button onClick={exportQuotePackage} className="rounded-lg border border-indigo-500 bg-white px-3 py-2 text-sm">Export Quote Package</button><button onClick={() => importQuotePackageInputRef.current?.click()} className="rounded-lg border border-indigo-500 bg-white px-3 py-2 text-sm">Import Quote Package</button><input ref={importQuotePackageInputRef} type="file" accept="application/json,.json" className="hidden" onChange={importQuotePackage} /></div><p className="mt-2 text-xs text-slate-600">This opens your email app with the quote request filled in. Please attach your exported design preview if needed.</p><div className="mt-3 rounded-md border border-slate-200 bg-white p-3"><p className="text-xs font-semibold text-slate-700">Suggested Subject Line</p><p className="mt-1 text-xs text-slate-600">{emailSubjectLine}</p><p className="mt-2 text-xs font-semibold text-slate-700">Email Summary Preview</p><pre className="mt-1 max-h-64 overflow-auto whitespace-pre-wrap rounded border bg-slate-50 p-2 text-[11px] leading-relaxed text-slate-700">{emailSummaryText || 'Click "Generate Email Summary" to build the message preview.'}</pre></div>{quotePackageStatus ? <p className="mt-2 text-xs text-slate-600">{quotePackageStatus}</p> : null}</section><button onClick={exportDesign} className="w-full rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white">Download PNG</button></aside>
<section className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200 md:p-4 lg:sticky lg:top-4 lg:h-[calc(100vh-7.5rem)] lg:flex lg:flex-col"><div className="mb-3 space-y-2 border-b pb-3"><div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap"><button onClick={() => restoreHistory(-1)} className="rounded-lg border px-3 py-2.5 text-sm">Undo</button><button onClick={() => restoreHistory(1)} className="rounded-lg border px-3 py-2.5 text-sm">Redo</button><button onClick={() => { const next = Math.max(0.5, zoom - 0.1); setZoom(next); fabricCanvasRef.current?.setZoom(next); }} className="rounded-lg border px-3 py-2.5 text-sm">-</button><span className="inline-flex items-center justify-center px-1 text-sm">{Math.round(zoom * 100)}%</span><button onClick={() => { const next = Math.min(2, zoom + 0.1); setZoom(next); fabricCanvasRef.current?.setZoom(next); }} className="rounded-lg border px-3 py-2.5 text-sm">+</button></div>{activeObject ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><button onClick={deleteSelected} className="rounded-lg border px-3 py-3 text-sm">Delete</button><button onClick={duplicateSelected} className="rounded-lg border px-3 py-3 text-sm">Duplicate</button><button onClick={() => moveLayer('forward')} className="rounded-lg border px-3 py-3 text-sm">Bring Fwd</button><button onClick={() => moveLayer('backward')} className="rounded-lg border px-3 py-3 text-sm">Send Back</button><button onClick={toggleLockSelected} className="rounded-lg border px-3 py-3 text-sm">{activeObject.selectable ? 'Lock' : 'Unlock'}</button><button onClick={() => alignSelected('horizontal')} className="rounded-lg border px-3 py-3 text-sm">Center H</button><button onClick={() => alignSelected('vertical')} className="rounded-lg border px-3 py-3 text-sm">Center V</button></div> : <p className="text-xs text-slate-500">Select an object to edit layer order, locking, and alignment.</p>}</div>
<div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_200px] lg:flex-1 lg:min-h-0"><div className="flex items-center justify-center p-1 md:p-4 lg:min-h-0"><div id="design-canvas" className="relative aspect-[420/520] w-full max-w-[760px]">{hasPreviewImage && resolvedImageUrl ? <img src={resolvedImageUrl} alt={`${selectedPreview.productName} ${selectedPreview.colorName}`} className="h-full w-full rounded-md object-contain" /> : <TshirtShape color={shirtColor} bodyPath={selectedProduct.mockups[shirtView]} view={shirtView} />}{showPrintArtboard ? <div className="pointer-events-none absolute rounded-md border border-indigo-400/35 bg-indigo-100/5" style={{ top: `${artboardPercent.top}%`, left: `${artboardPercent.left}%`, width: `${artboardPercent.width}%`, height: `${artboardPercent.height}%` }} /> : null}
<div className="pointer-events-none absolute border border-indigo-300/60" style={{ top: designArea.top + designArea.height / 2, left: designArea.left, width: designArea.width }} />
<div className="pointer-events-none absolute border border-indigo-300/60" style={{ top: designArea.top, left: designArea.left + designArea.width / 2, height: designArea.height }} />
{showPrintArtboard ? <div className="pointer-events-none absolute rounded-md border border-emerald-500/30" style={{ top: `${artboardPercent.top + 1.5}%`, left: `${artboardPercent.left + 1.5}%`, width: `${Math.max(0, artboardPercent.width - 3)}%`, height: `${Math.max(0, artboardPercent.height - 3)}%` }} /> : null}<div className="absolute overflow-hidden rounded-md" style={{ top: `${artboardPercent.top}%`, left: `${artboardPercent.left}%`, width: `${artboardPercent.width}%`, height: `${artboardPercent.height}%` }}><canvas ref={canvasElRef} className="h-full w-full touch-none" /></div></div></div><aside className="rounded-xl border bg-slate-50 p-2 lg:overflow-y-auto"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Layers</p><div className="space-y-1">{layers.length === 0 ? <p className="text-xs text-slate-500">No objects yet</p> : layers.map((layer) => <button key={layer.id} onClick={() => { const canvas = fabricCanvasRef.current; if (!canvas) return; const target = canvas.getObjects().find((obj) => (obj as FabricObject & { data?: { layerId?: string } }).data?.layerId === layer.id); if (!target) return; canvas.setActiveObject(target); canvas.requestRenderAll(); setActiveObject(target); refreshLayers(canvas); }} className={`flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-xs ${layer.isActive ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700'}`}><span>{layer.name}</span><span className="opacity-70">{layer.type}</span></button>)}</div></aside></div></section></div><div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-4 py-3 text-xs md:text-sm"><img src={getProductCardImage(selectedPreview)} alt={selectedProductName} className="h-10 w-10 rounded border bg-slate-100 object-cover" /><div className="min-w-0 flex-1"><p className="truncate font-semibold">{selectedProductName}</p><p className="truncate text-slate-600">Color: {selectedColorName} · Qty: {totalQuantity} · Est: ${estimatedPerShirt.toFixed(2)}/ea</p></div><button onClick={saveDraftToLocal} className="rounded-lg border px-3 py-2">Save</button><button onClick={copyQuoteSummary} className="rounded-lg bg-slate-900 px-3 py-2 text-white">Quote Request</button></div></div></main>;
}
