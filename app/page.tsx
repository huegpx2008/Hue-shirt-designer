...
export default function Home() {
  ...
  const isCustomCoro = isSheetPricedProduct && (selectedSignProduct.id !== 'yard-sign' || String(signValues.size || '') === 'custom');
  ...
  const signWidth = selectedSignProduct.id === 'yard-sign' ? isCustomCoro ? Number(signValues.width || firstSizedCustomCoroArtwork?.signWidth || 0) : selectedCoroSize.width : isSheetPricedProduct ? Number(signValues.width || firstSizedCustomCoroArtwork?.signWidth || 0) : selectedSignProduct.id === 'vehicle-magnet' ? isCustomMagnet ? Number(signValues.width || 0) : selectedMagnetSize.width : Number(signValues.width || 0);
  const signHeight = selectedSignProduct.id === 'yard-sign' ? isCustomCoro ? Number(signValues.height || firstSizedCustomCoroArtwork?.signHeight || 0) : selectedCoroSize.height : isSheetPricedProduct ? Number(signValues.height || firstSizedCustomCoroArtwork?.signHeight || 0) : selectedSignProduct.id === 'vehicle-magnet' ? isCustomMagnet ? Number(signValues.height || 0) : selectedMagnetSize.height : Number(signValues.height || 0);
  ...
  const coroUnusedSheetSpaces = isCustomCoro && selectedSignProduct.id === 'yard-sign' ? 0 : Math.max(0, (coroSheetLayout.sheetCount * coroSheetLayout.signsPerSheet) - effectiveCoroQuantity);
      ...
      <header className={`hue-site-header ${isProductionBuilder ? 'border-b border-white/10 bg-[#080d14]/96 px-5 py-3 shadow-[0_10px_32px_rgba(0,0,0,0.42)] backdrop-blur md:px-7' : 'border-b border-white/70 bg-white/90 px-4 py-3 shadow-[0_8px_30px_rgba(7,17,31,0.06)] backdrop-blur md:px-6'}`}>
        <div className={`hue-site-header-inner mx-auto flex max-w-[1800px] flex-wrap items-center gap-3 ${isProductionBuilder ? 'justify-between' : ''}`}>
          ...
          <div className="hue-mobile-header-actions order-2 flex w-full items-center gap-2 overflow-x-auto pb-1 text-sm md:order-none md:w-auto md:overflow-visible md:pb-0">
            ...
            {isProductionBuilder ? <div className="relative"><button type="button" onClick={() => setShowMainMenu((current) => !current)} aria-expanded={showMainMenu} className="rounded border border-white/20 bg-[#0b1018] px-4 py-2 font-bold text-white hover:border-slate-500">Menu</button>{showMainMenu ? <div className="fixed right-4 top-20 z-50 w-64 overflow-hidden rounded-2xl border border-[#38bdf8]/25 bg-[#071522] text-left shadow-[0_28px_80px_rgba(0,0,0,0.55),0_0_30px_rgba(14,165,233,0.16)] md:absolute md:right-0 md:top-full md:mt-2"><div className="border-b border-white/10 px-4 py-3"><p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#67d8ff]">Hue Studio Menu</p><p className="mt-1 text-xs text-slate-400">Quick places to jump if you get turned around.</p></div><div className="p-2"><button type="button" onClick={() => { setStoreView('store'); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Product Catalog<span className="mt-0.5 block text-xs font-normal text-slate-500">Browse banners, signs, apparel, and more</span></button><button type="button" onClick={() => { if (storeView === 'store') openStandaloneImageZone(); else openArtworkLibrary(); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Image Zone<span className="mt-0.5 block text-xs font-normal text-slate-500">Open saved artwork and uploads</span></button><button type="button" onClick={() => { openCanvaImport(); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Import Canva<span className="mt-0.5 block text-xs font-normal text-slate-500">Bring a Canva design into Image Zone</span></button><button type="button" onClick={() => { openCustomerAccount(); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">My Account<span className="mt-0.5 block text-xs font-normal text-slate-500">Sign in, create an account, or view saved artwork</span></button><button type="button" onClick={() => { setShowCart(true); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Cart &amp; Checkout<span className="mt-0.5 block text-xs font-normal text-slate-500">Review items and submit your order</span></button><a href="mailto:orders@huegraphics.com" onClick={() => setShowMainMenu(false)} className="block rounded-xl px-3 py-3 text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Help / Contact Hue<span className="mt-0.5 block text-xs font-normal text-slate-500">Ask us for help with artwork or ordering</span></a></div></div> : null}</div> : null}
          </div>
        </div>
      </header>
            ...
            <div className={`hue-mobile-builder-stage relative flex items-center justify-center ${isProductionBuilder ? 'coro-hex-bg h-full rounded-none p-0' : 'rounded-lg p-4'} ${productMode === 'signage' ? `${isProductionBuilder ? '' : 'min-h-[660px] bg-[linear-gradient(#e5e7eb_1px,transparent_1px),linear-gradient(90deg,#e5e7eb_1px,transparent_1px)] bg-[size:24px_24px]'} overflow-hidden bg-[#202224]` : 'min-h-[520px] overflow-hidden bg-[#e2e7ed]'}`}>
              ...
              <div id="design-canvas" className={`hue-builder-canvas ${isProductionBuilder ? `absolute inset-x-0 mx-auto w-full ${isCoroBuilder ? 'bottom-20 top-56' : 'bottom-20 top-60'}` : 'relative w-full'} ${productMode === 'signage' ? `${isProductionBuilder ? 'max-w-none' : 'mt-24 aspect-[4/3] max-w-[1040px]'}` : productMode === 'apparel' ? 'aspect-[420/520] max-w-[860px]' : 'aspect-[420/520] max-w-[760px]'}`}>
                  ...
                  {isCoroBuilder ? <div className={`coro-sheet-stage relative flex w-full items-center justify-center ${activeCoroOptionPanel === 'images' ? 'pl-[340px]' : ''}`}>
                    ...
                    <div className={`coro-sheet-carousel relative z-[1] flex items-center gap-20 overflow-x-auto px-12 pb-24 pt-16 ${activeCoroOptionPanel === 'images' ? 'max-w-[calc(100vw-460px)]' : 'max-w-[86vw]'}`}>
                    {coroSheetPreviews.map((sheetPreview, sheetIndex) => {
                      const selectedSheet = sheetIndex === activeCoroSheetIndex;
                      return <div key={sheetPreview.sheetNumber} onClick={() => setActiveCoroSheetIndex(sheetIndex)} className={`coro-sheet-shell relative flex shrink-0 cursor-pointer items-center justify-center transition ${selectedSheet ? 'z-10 scale-[1.03]' : 'opacity-75 hover:opacity-100'} ${activeCoroOptionPanel === 'images' ? 'w-[min(20vw,34vh)] min-w-52 max-w-[340px]' : coroSheetPreviews.length > 1 ? 'w-[min(22vw,36vh)] min-w-56 max-w-[360px]' : 'w-[min(24vw,40vh)] min-w-60 max-w-[390px]'}`} style={{ aspectRatio: CORO_SHEET.width / CORO_SHEET.height }}>
              ...
              {isProductionBuilder && activeCoroOptionPanel && activeCoroOptionPanel !== 'images' ? <div className="absolute bottom-20 left-1/2 z-50 w-[min(760px,92vw)] -translate-x-1/2 rounded-lg border border-slate-600 bg-[#f8fafc] p-4 text-slate-950 shadow-[0_24px_70px_rgba(0,0,0,0.45)]">
                ...
                {activeCoroOptionPanel === 'size' && isCoroBuilder && selectedSignProduct.id === 'yard-sign' ? <div className="mt-4 grid gap-4 lg:grid-cols-[240px_1fr]">
                  <div className="rounded border border-slate-200 bg-white p-3 text-center">
                    ...
                    <button type="button" onClick={switchCoroToCustomSize} className="mt-2 rounded bg-[#1678b8] px-4 py-2 text-sm font-black text-white hover:bg-[#0f5f94]">{isCustomCoro ? 'Custom Cut On' : 'Switch to Custom Cut'}</button>
                ...
                {activeCoroOptionPanel === 'size' && isCoroBuilder && selectedSignProduct.id !== 'yard-sign' ? <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <label className="text-xs font-bold uppercase tracking-wide text-slate-600">Width inches<input type="number" min={1} step="0.25" value={String(signValues.width ?? signWidth)} onChange={(event) => updateSignOption('width', event.target.value)} className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm font-normal text-slate-950" /></label>
                  <label className="text-xs font-bold uppercase tracking-wide text-slate-600">Height inches<input type="number" min={1} step="0.25" value={String(signValues.height ?? signHeight)} onChange={(event) => updateSignOption('height', event.target.value)} className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm font-normal text-slate-950" /></label>
                  <label className="text-xs font-bold uppercase tracking-wide text-slate-600">Quantity<input type="number" min={1} step={1} value={String(signValues.quantity ?? designerQuantity)} onChange={(event) => updateSignOption('quantity', event.target.value)} className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm font-normal text-slate-950" /></label>
