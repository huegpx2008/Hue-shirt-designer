...
export default function Home() {
  ...
  const filteredPreviewCatalog = useMemo(() => {
    const styleQuery = styleFilter.trim().toLowerCase();
    const nameQuery = nameFilter.trim().toLowerCase();
    const brandQuery = brandFilter.trim().toLowerCase();
    const colorQuery = colorFilter.trim().toLowerCase();
    return previewCatalog.filter((item) => {
      if (styleQuery && !item.styleNumber.toLowerCase().includes(styleQuery)) return false;
      if (nameQuery && !item.productName.toLowerCase().includes(nameQuery)) return false;
      if (brandQuery && !item.brand.toLowerCase().includes(brandQuery)) return false;
      if (colorQuery && !item.color.toLowerCase().includes(colorQuery)) return false;
      return true;
    });
  }, [brandFilter, colorFilter, nameFilter, previewCatalog, styleFilter]);
