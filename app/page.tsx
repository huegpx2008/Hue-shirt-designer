...
export default function Home() {
  ...
  const filteredPreviewCatalog = useMemo(() => {
    ...
    return groupedStyles.filter((group) => {
      ...
      return group.items.some((item) => [item.styleNumber, item.productName, item.brand, item.category || '', item.color, item.firstImageUrl || '', item.frontModelImageUrl || '', item.backModelImageUrl || '', item.frontFlatImageUrl || '', item.backFlatImageUrl || '', item.productImage || '']
        .some((value) => value.toLowerCase().includes(query)));
    });
