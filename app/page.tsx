...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-stone-100 pb-24 text-slate-950">
      <header className="border-b border-slate-200 bg-white px-4 py-3 md:px-6">
        <div className="mx-auto flex max-w-[1800px] flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <button onClick={exportDesign} className="rounded-md bg-teal-700 px-3 py-2 font-medium text-white hover:bg-teal-800">Download PNG</button>
            <a href="#quote" className="rounded-md bg-slate-950 px-3 py-2 font-medium text-white hover:bg-slate-800">Quote</a>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1800px] gap-4 px-4 py-4 md:px-6 xl:grid-cols-[300px_minmax(520px,1fr)_360px] xl:items-start">
        <aside id="product" className="rounded-lg border border-slate-200 bg-white shadow-sm xl:sticky xl:top-4 xl:max-h-[calc(100vh-8rem)] xl:overflow-hidden">
        ...
        <aside id="design" className="space-y-4 xl:sticky xl:top-4 xl:max-h-[calc(100vh-8rem)] xl:overflow-y-auto">
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-2"><input value={textValue} onChange={(event) => setTextValue(event.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-teal-700 focus:ring-2 focus:ring-teal-100" placeholder="Type your text" /><button onClick={addText} className="w-full rounded-md bg-slate-950 px-3 py-2 text-sm font-medium text-white hover:bg-slate-800">Add Text</button><label className="block cursor-pointer rounded-md border border-dashed border-slate-400 bg-slate-50 p-3 text-center text-sm font-medium hover:bg-slate-100">Upload Artwork<input onChange={onUploadImage} className="hidden" type="file" accept="image/*" /></label></div>
          </section>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 space-y-4">
              <div className="grid grid-cols-2 gap-2">{(['front', 'back'] as ShirtView[]).map((view) => <button key={view} onClick={() => setShirtView(view)} className={`rounded-md border px-3 py-2 text-sm font-medium capitalize ${shirtView === view ? 'border-teal-700 bg-teal-700 text-white' : 'border-slate-300 bg-white hover:bg-slate-50'}`}>{view}</button>)}</div>
              ...
              <div className="grid grid-cols-2 gap-2">{availableStyleColors.map((color) => <button key={color.name} type="button" onClick={() => { const match = selectedStyleItems.find((item) => item.colorName === color.name); if (match) { const idx = previewCatalog.findIndex((entry) => entry.styleNumber === match.styleNumber && entry.colorName === match.colorName); if (idx >= 0) setSelectedPreviewId(idx); } setShirtColor(color.value); }} className={`flex min-w-0 items-center gap-2 rounded-md border px-2 py-2 text-left text-xs ${shirtColor === color.value ? 'border-teal-700 bg-teal-50 ring-1 ring-teal-200' : 'border-slate-300 bg-white hover:bg-slate-50'}`} title={color.name}>{color.swatchUrl ? <img src={color.swatchUrl} alt={color.name} className="h-5 w-5 shrink-0 rounded-full border object-cover" /> : <span className="h-5 w-5 shrink-0 rounded-full border" style={{ background: color.value }} />}<span className="truncate">{color.name}</span></button>)}</div>
              <div className="grid grid-cols-2 gap-2">{(['flat', 'model'] as ImageType[]).map((type) => <button key={type} onClick={() => setImageType(type)} className={`rounded-md border px-3 py-2 text-sm font-medium ${imageType === type ? 'border-teal-700 bg-teal-700 text-white' : 'border-slate-300 bg-white hover:bg-slate-50'}`}>{type === 'flat' ? 'Flat' : 'Model'}</button>)}</div>
            </div>
          </section>
