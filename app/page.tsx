    ...
    <main className={`${isProductionBuilder ? `flex min-h-screen flex-col bg-[#050b12] text-slate-100 ${storeView === 'builder' ? 'overflow-y-auto pb-0 md:h-screen md:overflow-hidden' : 'overflow-y-auto pb-12'}` : 'min-h-screen bg-[#f4f8fc] pb-24 text-slate-950'}`}>
      ...
      {isProductionBuilder && showMainMenu ? <div className="fixed inset-0 z-[9999]" onClick={() => setShowMainMenu(false)}>
        <div className="absolute right-4 top-20 w-72 overflow-hidden rounded-2xl border border-[#38bdf8]/25 bg-[#071522] text-left text-white shadow-[0_28px_80px_rgba(0,0,0,0.65),0_0_34px_rgba(14,165,233,0.18)]" onClick={(event) => event.stopPropagation()}>
          ...
          <div className="p-2">
            <button type="button" onClick={() => { setStoreView('store'); setShowMainMenu(false); }} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Product Catalog<span className="mt-0.5 block text-xs font-normal text-slate-500">Browse banners, signs, apparel, and more</span></button>
            ...
            <a href="mailto:orders@huegraphics.com" onClick={() => setShowMainMenu(false)} className="block rounded-xl px-3 py-3 text-sm font-bold text-slate-100 hover:bg-white/[0.07]">Help / Contact Hue<span className="mt-0.5 block text-xs font-normal text-slate-500">Ask us for help with artwork or ordering</span></a>
          </div>
        </div>
      </div> : null}
