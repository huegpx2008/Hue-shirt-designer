  ...
  return <div className="px-5 py-4">
    ...
    {open ? <div className="mt-4 space-y-3 border-t border-white/10 pt-4">
      ...
      <div className="grid gap-3 xl:grid-cols-3">
        ...
        <section className="rounded-xl border border-white/10 bg-[#020a12] p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#67d8ff]">Fulfillment & payment</p><div className="mt-3 space-y-1 text-xs leading-5 text-slate-300"><p className="font-black text-white">{fulfillment?.method === 'direct_ship' ? 'Direct shipping' : 'Local pickup'}</p>{fulfillment?.method === 'direct_ship' ? <><p>{address?.line1 || 'Address not provided'}</p>{address?.line2 ? <p>{address.line2}</p> : null}<p>{[address?.city, address?.state, address?.postalCode].filter(Boolean).join(', ')}</p></> : <p>Customer will pick up at Hue Graphics.</p>}<p className="pt-2 text-slate-500">Submitted {date(order.created_at)}</p><p className="text-slate-500">Order status: {order.status || order.order_data?.status || 'received'}</p><p className={order.payment_status === 'completed' ? 'font-bold text-green-300' : 'text-slate-500'}>Payment: {order.payment_provider === 'paypal' ? `PayPal · ${order.payment_status || 'pending'}` : order.order_data?.paymentMode || 'Not recorded'}</p>{order.paid_at ? <p className="text-slate-500">Paid {date(order.paid_at)}</p> : null}{order.paypal_order_id ? <p className="break-all text-[10px] text-slate-500">PayPal order: {order.paypal_order_id}</p> : null}{order.paypal_capture_id ? <p className="break-all text-[10px] text-slate-500">Capture: {order.paypal_capture_id}</p> : null}</div></section>
