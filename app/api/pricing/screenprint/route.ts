...
export async function POST(request: Request) {
  ...
  const retryAfter = enforceRateLimit(request, 'pricing-screenprint', 120, 60 * 1000);
  ...
  try {
    ...
    const studioData = response.ok ? await applyStudioPricingAdjustment(data, 'screenprint') : data;
