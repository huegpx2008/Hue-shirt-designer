...
export async function POST(request: Request) {
  try {
    ...
    const existingRows = await supabaseAdminFetch(`/rest/v1/hue_payment_attempts?submission_key=eq.${encodeURIComponent(checkout.submissionKey)}&select=paypal_order_id,paypal_capture_id,status,paid_at&limit=1`) as Array<{
    ...
    if (existing?.status === 'completed' && existing.paypal_order_id === checkout.paypalOrderId && existing.paypal_capture_id) {
      const paymentToken = createPayPalToken({
        kind: 'paypal_payment', paypalOrderId: checkout.paypalOrderId, captureId: existing.paypal_capture_id,
        submissionKey: checkout.submissionKey, amount: checkout.amount, currency: checkout.currency,
        customerEmail: checkout.customerEmail, exp: Math.floor(Date.now() / 1000) + 30 * 60,
      });
    ...
    const paymentToken = createPayPalToken({
      kind: 'paypal_payment', paypalOrderId: checkout.paypalOrderId, captureId: completed.captureId,
      submissionKey: checkout.submissionKey, amount: checkout.amount, currency: checkout.currency,
      customerEmail: checkout.customerEmail, exp: Math.floor(Date.now() / 1000) + 30 * 60,
    });
