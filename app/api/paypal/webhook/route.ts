...
export async function POST(request: Request) {
  try {
    ...
    if (status && (paypalOrderId || captureId)) {
      const filter = paypalOrderId
        ? `paypal_order_id=eq.${encodeURIComponent(paypalOrderId)}`
        : `paypal_capture_id=eq.${encodeURIComponent(String(captureId))}`;
      const update = { status, webhook_data: event, updated_at: new Date().toISOString(), ...(captureId ? { paypal_capture_id: captureId } : {}) };
      ...
      await supabaseAdminFetch(`/rest/v1/hue_orders?${filter}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ payment_status: status, payment_data: event, updated_at: new Date().toISOString() }) });
    }
