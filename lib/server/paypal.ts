...
export const capturePayPalOrder = async (paypalOrderId: string, submissionKey: string) => paypalRequest<PayPalOrderResponse>(
  ...
  { method: 'POST', headers: { 'PayPal-Request-Id': `hue-capture-${submissionKey}`.slice(0, 108) }, body: '{}' },
);

export const validateCompletedCapture = (payload: PayPalOrderResponse, expected: {
  paypalOrderId: string;
  submissionKey: string;
  amount: string;
  currency: string;
