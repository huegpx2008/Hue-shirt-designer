...
- Supabase stores customer authentication, artwork metadata, private medium working previews, and small thumbnails. Backblaze B2 stores active production originals, and verified ordered originals are archived to Google Drive. Public catalog and website imagery may be delivered by Vercel, Cloudinary, or the SanMar CDN depending on the asset.
...
| [Supabase](https://supabase.com/dashboard) | Customer authentication, Image Zone metadata, and private previews | Plan not recorded here | The app uses Supabase Auth, the `artwork-files` storage bucket, and `hue_artwork_assets`. Apply `supabase/storage-policies.sql` and `supabase/hue-studio-b2-artwork-assets.sql`. |
| [Backblaze B2](https://secure.backblaze.com/b2_buckets.htm) | Private active production originals | Plan not recorded here | Use a private bucket and a bucket-restricted S3 application key. Browser uploads use short-lived signed PUT URLs; B2 credentials never reach the browser. |
| [Google Drive](https://drive.google.com/) | Verified long-term ordered production archive | Plan not recorded here | Order originals are streamed from B2, size-verified in Drive, and retained in B2 for a safety window before cleanup. |
